...
import { Timeline, TimelineEvent, EventSourceType, TimelineCluster, TimelineGap } from '../../types/biography';
...
export class TimelineService {
     ...
     */
    async constructTimeline(userId: string): Promise<Timeline> {
        logger.info(`Constructing timeline for user ${userId}`);
        ...
        logger.info(`Timeline constructed with ${sortedEvents.length} events, ${clusters.length} clusters, and ${gaps.length} gaps`);
        return timeline;
    }
     ...
     */
    private async fetchAllEvents(userId: string): Promise<TimelineEvent[]> {
        ...
        for (const content of contents) {
            timelineEvents.push({
                id: content.id,
                userId,
                sourceType: EventSourceType.POST,
                sourceId: content.id,
        ...
        for (const email of emails) {
            timelineEvents.push({
                id: email.id,
                userId,
                sourceType: EventSourceType.EMAIL,
                sourceId: email.id,
        ...
        for (const item of media) {
            ...
            timelineEvents.push({
                id: item.id,
                userId,
                sourceType: EventSourceType.MEDIA,
                sourceId: item.id,
     ...
     */
    private generateClusters(events: TimelineEvent[]): TimelineCluster[] {
        ...
        let currentClusterEvents: TimelineEvent[] = [events[0]];
    ...
    private createClusterObject(events: TimelineEvent[]): TimelineCluster {
        return {
     ...
     */
    async enrichTimeline(timeline: Timeline): Promise<Timeline> {
        logger.info(`Enriching timeline for user ${timeline.userId} with AI categorization`);

        // Categorize all events in batches
        const categorizationResults = await categorizationService.categorizeBatch(timeline.events);
        ...
        logger.info(`Enriched ${timeline.events.length} events`);
        return timeline;
    }
}
