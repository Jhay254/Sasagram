...
export class NotificationService {
     ...
     */
    async sendTagNotification(tagId: string): Promise<void> {
        ...
        if (!tag) {
            logger.warn(`Tag ${tagId} not found for notification`);
            return;
        }
        ...
        logger.info(`Tag notification sent to ${tag.taggedUser.email}`, {
            tagId,
    ...
    async sendInviteNotification(
        ...
        if (!tagger) {
            logger.warn(`Tagger ${taggerId} not found for invite`);
            return;
        }
        ...
        logger.info(`Invite notification sent to ${email}`, {
            from: tagger.email,
     ...
     */
    private buildTagNotificationEmail(tag: any): EmailTemplate {
