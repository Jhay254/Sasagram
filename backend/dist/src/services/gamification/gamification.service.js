...
class GamificationService {
     ...
     */
    async checkAchievements(userId, triggerType, value = 1) {
        try {
    ...
    async unlockAchievement(userId, achievementId) {
        try {
            ...
            logger_1.default.info(`Achievement unlocked for user ${userId}: ${achievementId}`);
        }
        catch (error) {
            // Ignore unique constraint violations (already unlocked)
            if (error.code !== 'P2002') {
                logger_1.default.error('Error unlocking achievement:', error);
            }
        }
    }
    /**
